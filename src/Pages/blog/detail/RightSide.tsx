...
const RightSide = ({ data, isLoading }: any) => {
  ...
  const checkPrevious = useCallback(async () => {
    try {
      const getData = await getDetailBlogData({
        filters: { id: { eq: Number(data.id) - 1 } },
      });
      if (
        getData?.voyagerArticles?.data &&
        getData?.voyagerArticles?.data?.length > 0
      ) {
        setPreviousData(getData?.voyagerArticles?.data?.[0].attributes);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const checkNext = useCallback(async () => {
    try {
      ...
      if (
        ...
        setNextData(getData?.voyagerArticles?.data?.[0]?.attributes);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <div className="flex flex-col justify-between gap-8 w-full">
      ...
      {isLoading ? (
        <div className="w-full py-8 grid grid-cols-12 gap-4">
        ...
        <div className="border-t border-b py-8 flex flex-col gap-6">
          <div className="flex justify-between items-center w-full">
            <div className="flex flex-col">
              ...
              <p className="text-gray-500 text-sm">
                ...
                {data?.attributes?.minutes_read} min read
              </p>
            </div>
            <div className="flex gap-2">
              {data?.attributes?.categories?.data?.map((category: any) => (
                <div
          ...
          />
          <Markdown className="text-gray-600 text-lg">
